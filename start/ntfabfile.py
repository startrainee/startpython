...
from fabric.api import env, run, local, put, cd, settings, get, lcd
from fabric.operations import sudo

env.user = 'root'
env.sudo_user = 'root'
# env.hosts = ['47.95.242.139']    #  多个服务器部署
env.host_string = '47.95.242.139'  # 改成你的服务器ip
env.port = '22'
env.password = "wcywcy9999."
db_user = 'subadmin'
db_password = 'subadmin'
_TAR_FILE = 'dist-mypythonblog.tar.gz'
_REMOTE_TMP_TAR = '/tmp/%s' % _TAR_FILE
_REMOTE_BASE_DIR = '/srv/mypythonblog'
...
def deploy():
    print("deploy is start......")
    newdir = 'www-%s' % _now()
    run('rm -f %s' % _REMOTE_TMP_TAR)
    put('dist/%s' % _TAR_FILE, _REMOTE_TMP_TAR)
    with cd(_REMOTE_BASE_DIR):
        print("_REMOTE_BASE_DIR is start......")
        sudo('mkdir %s' % newdir)
    with cd('%s/%s' % (_REMOTE_BASE_DIR, newdir)):
        print("_REMOTE_BASE_DIR, newdir is start......")
        ...
        sudo('todos app.py')  # 解决windows和linux行尾换行不同问题
    ...
    with settings(warn_only=True):
        print("warn_only is start......")
        sudo('supervisorctl stop mypythonblog')  # supervisor重启app
        sudo('supervisorctl start mypythonblog')
        sudo('/etc/init.d/nginx reload')  # nginx重启

    print("deploy is over!!!!")
...
def rollback():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        with settings(warn_only=True):
            sudo('supervisorctl stop mypythonblog')
            sudo('supervisorctl start mypythonblog')
...
def backup():
    ...
    '''
    dt = _now()
    f = 'backup-mypythonblog-%s.sql' % dt
    with cd('/tmp'):
        run(
            'mysqldump --user=%s --password=%s --skip-opt --add-drop-table --default-character-set=utf8 --quick mypythonblog > %s' % (
...
def restore2local():
    ...
    sqls = [
        'drop database if exists mypythonblog;',
        'create database mypythonblog;',
        'alter database mypythonblog default character set utf8 collate utf8_general_ci;'  # 修改为utf8字符集
        'grant select, insert, update, delete on mypythonblog.* to \'%s\'@\'localhost\' identified by \'%s\';' % (
            db_user, db_password)
    ]
    for sql in sqls:
    ...
    with lcd('backup'):
        ...
        local(r'mysql -uroot -p%s --default-character-set=utf8 mypythonblog < %s' % (p, restore_file[:-7]))  # 导入数据库
