        ...
        showFormLoading: function (isLoading) {
            return this.each(function () {
                ...
                if (isLoading) {
                    ...
                    $span && $span.addClass('glyphicon-refresh').addClass('glyphicon-refresh');
                } else {
                    $buttons.removeAttr('disabled');
                    $span && $span.removeClass('glyphicon-refresh').removeClass('glyphicon-refresh');
                }
            });
        },
...
function _bindSubmit($form) {
    $form.submit(function (event) {
        ...
        var
            fn_error = $form.attr('error'),
            fn_success = $form.attr('success'),
            fn_data = $form.attr('data'),
        ...
        $i && $i.addClass('glyphicon-refresh').addClass('glyphicon-refresh');
        postJSON($form.attr('action-url'), data, function (err, result) {
            $i && $i.removeClass('glyphicon-refresh').removeClass('glyphicon-refresh');
            if (err) {
